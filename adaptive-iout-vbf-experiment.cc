...
static void
GenerateTraffic (Ptr<AquaSimAdaptiveVbf> routing, uint32_t *seq,
                 uint32_t payloadBytes, double interval, double stopTime,
                 Ptr<UniformRandomVariable> jitter)
{
  if (Simulator::Now ().GetSeconds () >= stopTime) return;
  routing->OriginatePacket ((*seq)++, payloadBytes);
  double next = interval + jitter->GetValue (-0.1 * interval, 0.1 * interval);
  Simulator::Schedule (Seconds (next), &GenerateTraffic, routing, seq,
                       payloadBytes, interval, stopTime, jitter);
}
...
main (int argc, char *argv[])
{
  ...
  std::vector<uint32_t> seqCounters (nNodes, 0);

  for (uint32_t s = 1; s <= sources; s++)
    {
      /* Spread sources across the node set rather than clustering at low IDs. */
      uint32_t idx = 1 + (s - 1) * ((nNodes - 1) / sources);
      if (idx >= nNodes) idx = nNodes - 1;

      if (protocol == "adaptive" && aivRouting[idx] != 0)
        {
          Simulator::Schedule (Seconds (5.0 + 0.37 * s), &GenerateTraffic,
                               aivRouting[idx], &seqCounters[idx],
                               payloadBytes, packetInterval, simTime, jitter);
        }
      else
        {
          /* Baselines use the standard socket path. */
          Ptr<Socket> sock = Socket::CreateSocket (
              nodes.Get (idx), TypeId::LookupByName ("ns3::PacketSocketFactory"));
          PacketSocketAddress addr;
          addr.SetAllDevices ();
          addr.SetPhysicalAddress (devices.Get (0)->GetAddress ());
          addr.SetProtocol (0);
          sock->Connect (addr);

          OnOffHelper onoff ("ns3::PacketSocketFactory", Address (addr));
          onoff.SetConstantRate (
              DataRate (static_cast<uint64_t> (payloadBytes * 8 / packetInterval)),
              payloadBytes);
          ApplicationContainer app = onoff.Install (nodes.Get (idx));
          app.Start (Seconds (5.0 + 0.37 * s));
